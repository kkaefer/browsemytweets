...
exports.handle = function() {
  if (this.params.post.tweets) {
    var tweets = JSON.parse(this.params.post['tweets']);
    this.screen_name = JSON.parse(this.params.post['screen_name']);
    if (tweets) {
      tweets.forEach(addTweet, this);
      this.respond(200, JSON.stringify({ status: 'ok' }));
      return;
    }
  }

  this.respond(200, JSON.stringify({ status: "", message: "Couldn't parse tweets." }));
};
