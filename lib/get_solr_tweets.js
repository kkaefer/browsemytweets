...
exports.handle = function(userid) {
  ...
  solr_client.query(query, options, function(i, data){
    if(data) {
      ...
      self.respond(200, JSON.stringify(response));
      return
    }
    self.respond(500, JSON.stringify({ status: "", message: "Something went wrong with Solr. No data fetched." }));
  });
};